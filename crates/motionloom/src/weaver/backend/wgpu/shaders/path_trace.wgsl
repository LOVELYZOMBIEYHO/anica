...
struct Params { v: array<vec4<f32>, 20> }
struct Film { sum: vec4<f32>, stats: vec4<f32>, albedo: vec4<f32>, normal: vec4<f32> }
...
fn texel(desc: vec4<f32>, xy: vec2<i32>) -> vec4<f32> {
    let wh = vec2<i32>(desc.yz);
    let q = ((xy % wh) + wh) % wh;
    // Texture offsets use their raw u32 bits, avoiding f32's 24-bit integer limit.
    return unpack4x8unorm(pixels[bitcast<u32>(desc.x)+u32(q.y*wh.x+q.x)]);
}
fn texture(desc: vec4<f32>, uv: vec2<f32>, srgb: bool) -> vec4<f32> {
    if desc.y < 1.0 { return vec4<f32>(1.0); }
    let q = uv*desc.yz-0.5; let i = vec2<i32>(floor(q)); let f = fract(q);
    var a=texel(desc,i); var b=texel(desc,i+vec2<i32>(1,0)); var c=texel(desc,i+vec2<i32>(0,1)); var d=texel(desc,i+vec2<i32>(1,1));
...
fn intersect(o:vec3<f32>, d:vec3<f32>, limit:f32) -> Hit {
...
struct Surface { n:vec3<f32>, gn:vec3<f32>, base:vec3<f32>, alpha:f32, metal:f32, rough:f32, f0:vec3<f32>, emission:vec3<f32> }
fn surface(hit:Hit, d:vec3<f32>) -> Surface {
    let m=u32(p.v[1].y)+u32(scene[hit.tri+15u].x)*10u;
    let uv=interp(hit.tri,3u,hit.uv).xy;
    let color=interp(hit.tri,4u,hit.uv)*texture(scene[m+4u],uv,true);
    let mr=texture(scene[m+6u],uv,false);
    ...
    if dot(t,t)>0.5 {
        var map=texture(scene[m+5u],uv,false).xyz*2.0-1.0; map=vec3<f32>(map.xy*scene[m].z,map.z);
        n=unit(t*map.x+cross(n,t)*tangent.w*map.y+n*map.z,gn);
    }
    if dot(n,-d)<=0.0 { n=gn; }
    let metal=clamp(scene[m].x*mr.b,0.0,1.0); let rough=clamp(scene[m].y*mr.g,0.03,1.0);
    var alpha=1.0;
    if scene[m].w==1.0 { alpha=select(0.0,1.0,color.a>=scene[m+3u].x); }
    if scene[m].w==2.0 { alpha=color.a; }
    return Surface(n,gn,color.rgb,alpha,metal,rough,mix(0.04*scene[m+2u].rgb*scene[m+2u].w,color.rgb,metal),scene[m+1u].rgb*scene[m+1u].w*texture(scene[m+7u],uv,true).rgb);
}
...
fn env_texel(q:vec2<i32>)->vec3<f32> {
    ...
    return scene[u32(p.v[10].x)+u32(y*wh.x+x)].rgb;
}
...
fn env_pdf(d:vec3<f32>)->f32 {
    ...
    return scene[u32(p.v[13].x)+cell.y*wh.x+cell.x].y;
}
fn env_sample()->vec3<f32> {
    ...
    loop {if lo>=hi {break;}let mid=(lo+hi)/2u;if scene[u32(p.v[13].x)+mid].x<u {lo=mid+1u;}else{hi=mid;}}
...
fn visible(o:vec3<f32>, d:vec3<f32>, distance:f32)->f32 {
    var origin=o; var remain=distance; var trans=exp(-p.v[16].w*medium_segment(o,d,distance).y);
    for(var i=0u;i<u32(p.v[9].w);i++) {
        let h=intersect(origin,d,remain); if h.tri==0xffffffffu { return trans; }
...
struct Sample { color:vec3<f32>, albedo:vec3<f32>, normal:vec3<f32>, depth:f32 }
fn trace(origin:vec3<f32>, direction:vec3<f32>)->Sample {
    ...
    var first=Sample(vec3<f32>(0),vec3<f32>(0),vec3<f32>(0),0.0);
    ...
    loop {
        ...
        let h=intersect(o,d,INF);
        let segment=medium_segment(o,d,h.t);
        if segment.y>0.0 {
            let free_path=-log(max(1.0-random(),1e-7))/p.v[16].w;
            if free_path<segment.y {
                ...
                for(var li=0u;li<u32(p.v[1].w);li++) {
                    let at=u32(p.v[1].z)+li*4u;let light=scene[at];let settings=scene[at+1u];
                ...
                radiance+=throughput*env(l)*f*visible(point,l,INF)*power(pdf,f)/pdf;
                let next=phase_sample(d);prev_pdf=phase(dot(d,next));prev_emitter_nee=false;o=point;d=next;bounce++;
        ...
        if h.tri==0xffffffffu {
            let weight=select(power(prev_pdf,env_pdf(d)),1.0,bounce==0u);
            let intensity=select(p.v[11].x,p.v[11].y,bounce==0u);
            radiance+=throughput*env_radiance(d)*weight*intensity;
        ...
        if !finite3(s.n) || !finite3(s.gn) {return Sample(vec3<f32>(bitcast<f32>(0x7fc00000u)),vec3<f32>(10),s.n,f32(h.tri));}
        if random()>s.alpha {
            transparent++; if transparent>=u32(p.v[9].w) { break; }
            o=point+d*0.0002; continue;
        }
        if bounce==0u { first.albedo=s.base; first.normal=s.n; first.depth=h.t; }
        ...
        if !finite3(radiance) {return Sample(radiance,vec3<f32>(20),throughput,f32(h.tri));}
        if p.v[14].y>0.0 {
            var lo=0u;var hi=u32(p.v[14].y)-1u;let choice=random();
            loop {if lo>=hi {break;}let mid=(lo+hi)/2u;if scene[u32(p.v[14].x)+mid].x<choice {lo=mid+1u;}else{hi=mid;}}
            let tri=u32(scene[u32(p.v[14].x)+lo].y);let u=sqrt(random());let v=random();let uv=vec2<f32>(u*(1.0-v),u*v);
            ...
            if !finite3(radiance) {return Sample(radiance,vec3<f32>(30),vec3<f32>(pdf,f.a,distance),f32(tri));}
        }
        ...
        for(var li=0u;li<u32(p.v[1].w);li++) {
            ...
            if !finite3(radiance) {return Sample(radiance,vec3<f32>(40),f.rgb,f32(li));}
        }
        ...
        if ef.a>0.0 { radiance+=throughput*ef.rgb*max(dot(s.n,light_dir),0.0)*env(light_dir)*visible(point+s.gn*0.0002,light_dir,INF)*power(light_pdf,ef.a)/light_pdf; }
        if !finite3(radiance) {return Sample(radiance,vec3<f32>(50),vec3<f32>(light_pdf,ef.a,f32(bounce)),f32(h.tri));}
        var next=vec3<f32>(0);
        if random()<probability(s) {
            ...
            next=reflect(d,hh);
        } else { diffuse++; if diffuse>u32(p.v[8].y) { break; } next=cosine(s.n); }
        let f=bsdf(s,-d,next); if f.a<1e-10 { break; }
        throughput*=f.rgb*max(dot(s.n,next),0.0)/f.a; prev_pdf=f.a;prev_emitter_nee=true;
        if !finite3(throughput) {return Sample(throughput,vec3<f32>(60),f.rgb,f.a);}
        bounce++;
